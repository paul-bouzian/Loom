...
fn resolve_head_context_prioritizes_fork_selector_when_tracking_fork_remote() {
    ...
    let repo = harness
        .create_repo(
            "fuzzy-tiger",
            Some(("feature", "git@github.com:alice/threadex.git")),
        )
    ...
    assert_eq!(
        ...
        PullRequestHeadContext {
            ...
            head_repository_name_with_owner: Some("alice/threadex".to_string()),
...
fn resolver_prefers_open_pull_requests_and_ignores_closed_only_matches() {
    ...
    let repo = harness
        .create_repo(
            "fuzzy-tiger",
            Some(("origin", "git@github.com:acme/threadex.git")),
        )
        .expect("repo should be created");
    let project = harness
        .workspace
...
impl PullRequestHarness {
    fn new() -> Result<Self, Box<dyn std::error::Error>> {
        let temp_root = std::env::temp_dir().join(format!("threadex-pr-test-{}", Uuid::now_v7()));
    ...
    fn create_repo(
        ...
        git::run_git(
            ...
            [
                ...
                "add",
                "origin",
                "git@github.com:acme/threadex.git",
            ],
        )?;
