...
#[tokio::test]
async fn user_input_responses_can_continue_into_a_proposed_plan() {
    let (session, harness) = FakeCodexHarness::new().await;
    let runtime_context = context(
        "thread-local-user-input-plan",
        None,
        CollaborationMode::Plan,
        ApprovalPolicy::AskToEdit,
    );
    ...
    session
        .send_message(
            runtime_context.clone(),
            "Draft the migration plan".to_string(),
            Vec::new(),
        )
        .await
    ...
    let interaction_snapshot = wait_for_snapshot(
        &session,
        ThreadRuntimeContext {
            codex_thread_id: Some("thr-new".to_string()),
            ..runtime_context.clone()
        },
        |snapshot| !snapshot.pending_interactions.is_empty(),
    )
    .await;

    let interaction = match &interaction_snapshot.pending_interactions[0] {
        crate::domain::conversation::ConversationInteraction::UserInput(interaction) => interaction,
        other => panic!("expected user input interaction, got {other:?}"),
    };
    ...
    let answered_snapshot = session
        .respond_to_user_input_request(
            crate::domain::conversation::RespondToUserInputRequestInput {
                thread_id: "thread-local-user-input-plan".to_string(),
                interaction_id: interaction.id.clone(),
                answers: std::collections::HashMap::from([(
                    "scope".to_string(),
    ...
    assert!(answered_snapshot.pending_interactions.is_empty());
    ...
    harness
        .emit_notification(
            ...
            json!({
                ...
                "turnId": "turn-live-1",
                "item": {
                    "id": "assistant-plan-from-user-input",
                    "type": "agentMessage",
    ...
    harness
        .emit_notification(
            ...
            json!({
                "threadId": "thr-new",
                "turnId": "turn-live-1",
                "itemId": "assistant-plan-from-user-input",
                "delta": "\n<proposed_plan>\n# Migration Plan\n\n1. Inspect runtime\n2. Port shell\n"
            }),
        )
        .await;
        ...
        .emit_notification(
            ...
            json!({
                ...
                "item": {
                    "id": "assistant-plan-from-user-input",
                    "type": "agentMessage",
                    "text": "I have enough context now.\n<proposed_plan>\n# Migration Plan\n\n1. Inspect runtime\n2. Port shell\n</proposed_plan>\nLet's proceed once you approve it."
                }
            }),
        )
        .await;
    ...
    let snapshot = wait_for_snapshot(
        ...
        |snapshot| {
            snapshot.proposed_plan.as_ref().is_some_and(|plan| {
                plan.is_awaiting_decision
                    && plan.status == crate::domain::conversation::ProposedPlanStatus::Ready
                    && plan.markdown.contains("# Migration Plan")
            }) && snapshot.items.iter().any(|item| {
                matches!(
                    item,
                    crate::domain::conversation::ConversationItem::Message(message)
                        if message.id == "assistant-plan-from-user-input"
                            && message.text.contains("I have enough context now.")
                            && !message.text.contains("<proposed_plan>")
                )
            })
        },
    )
